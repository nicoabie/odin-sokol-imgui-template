...
INVALID_INDEX :: -1
SCENE_MAX_BUFFERS :: 1024
SCENE_MAX_IMAGES :: 512
...
gltf_to_vertex_format :: proc(acc: ^cgltf.accessor) -> sg.Vertex_Format {
	...
	case .r_16:
		...
		case .vec2:
			return bool(acc.normalized) ? .SHORT2N : .SHORT2
		case .vec4:
			return bool(acc.normalized) ? .SHORT4N : .SHORT4
		}
	case .r_32f:
		#partial switch acc.type {
		case .scalar:
...
create_vertex_buffer_mapping_for_gltf_primitive :: proc(
	gltf: ^cgltf.data,
...
create_sg_image_samplers_for_gltf_image :: proc "c" (
	gltf_image_index: i32,
	data: sg.Range,
	scene: ^Scene,
) {
	for _, i in scene.images {
		...
		if p.gltf_image_index == gltf_image_index {
			if p.has_basisu {
				...
				scene.images[i].tex_view = sg.make_view({texture = {image = scene.images[i].img}})
			} else {
				...
				scene.images[i].img = sg.make_image(
					{
						...
						data = {
							mip_levels = {
								0 = {ptr = pixels, size = uint(width * height * desired_channels)},
							},
						},
					},
				)
...
create_sg_layout_for_gltf_primitive :: proc(
	gltf: ^cgltf.data,
	prim: ^cgltf.primitive,
	vbuf_map: ^Vertex_Buffer_Mapping,
) -> sg.Vertex_Layout_State {
	layout: sg.Vertex_Layout_State
...
create_sg_pipeline_for_gltf_primitive :: proc(
	gltf: ^cgltf.data,
	prim: ^cgltf.primitive,
	vbuf_map: ^Vertex_Buffer_Mapping,
	scene: ^Scene,
		...
		sg.make_pipeline(
			{
				...
				cull_mode = .BACK,
				face_winding = .CCW,
				depth = {write_enabled = !pip_params.alpha, compare = .LESS_EQUAL},
				colors = {
					0 = {
	...
	for mesh_index in 0 ..< len(gltf.meshes) {
		...
		for prim_index in 0 ..< len(gltf_mesh.primitives) {
			...
			if gltf_prim.indices != nil {
				prim.index_buffer = i32(
					cgltf.buffer_view_index(gltf, gltf_prim.indices.buffer_view),
				)
				prim.base_element = 0
...
gltf_parse_images :: proc(gltf: ^cgltf.data, scene: ^Scene) -> ParseImagesResult {
	...
	for gltf_tex in gltf.textures {
		...
		p.min_filter =
			gltf_tex.sampler != nil ? gltf_to_sg_min_filter(gltf_tex.sampler.min_filter) : .LINEAR
		p.mag_filter =
			gltf_tex.sampler != nil ? gltf_to_sg_mag_filter(gltf_tex.sampler.mag_filter) : .LINEAR
		p.mipmap_filter =
