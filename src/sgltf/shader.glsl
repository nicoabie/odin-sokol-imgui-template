...
@block fs_shared_uniforms

// TODO rename to material_params
...
layout(binding=5) uniform texture2D specular_tex;
...
layout(binding=5) uniform sampler specular_smp;


@end

// acc assetto corsa car
@fs acc_fs
@include_block fs_shared_uniforms
...
void main() {
    ...
    // frag_color = vec4(uv, 0.0, 1.0) + vec4(result * 0.01, 0.0);
    // frag_color = vec4(v_uv, 0.0, 1.0) + vec4(result * 0.01, 0.0);
