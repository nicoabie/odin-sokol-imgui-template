...
when ODIN_OS == .Windows {
    when USE_DLL {
        ...
        when USE_GL {
            when DEBUG { foreign import lib { "sokol_fetch_windows_x64_gl_debug.lib" } }
            else       { foreign import lib { "sokol_fetch_windows_x64_gl_release.lib" } }
        } else {
            when DEBUG { foreign import lib { "sokol_fetch_windows_x64_d3d11_debug.lib" } }
            else       { foreign import lib { "sokol_fetch_windows_x64_d3d11_release.lib" } }
        }
    }
    ...
    when USE_DLL {
        ...
        when USE_GL {
            when ODIN_ARCH == .arm64 {
                when DEBUG { foreign import lib { "sokol_fetch_macos_arm64_gl_debug.a" } }
                else       { foreign import lib { "sokol_fetch_macos_arm64_gl_release.a" } }
            } else {
                when DEBUG { foreign import lib { "sokol_fetch_macos_x64_gl_debug.a" } }
                else       { foreign import lib { "sokol_fetch_macos_x64_gl_release.a" } }
            }
        } else {
            when ODIN_ARCH == .arm64 {
                when DEBUG { foreign import lib { "sokol_fetch_macos_arm64_metal_debug.a" } }
                else       { foreign import lib { "sokol_fetch_macos_arm64_metal_release.a" } }
            } else {
                when DEBUG { foreign import lib { "sokol_fetch_macos_x64_metal_debug.a" } }
                else       { foreign import lib { "sokol_fetch_macos_x64_metal_release.a" } }
            }
        }
    }
} else when ODIN_OS == .Linux {
    when USE_DLL {
        when DEBUG { foreign import lib { "sokol_fetch_linux_x64_gl_debug.so" } }
        else       { foreign import lib { "sokol_fetch_linux_x64_gl_release.so" } }
    } else {
        when DEBUG { foreign import lib { "sokol_fetch_linux_x64_gl_debug.a" } }
        else       { foreign import lib { "sokol_fetch_linux_x64_gl_release.a" } }
    }
...
sfetch_error_t :: enum u32 {
	...
	CANCELLED           = 6,
	JS_OTHER            = 7, // check browser console for detailed error info
}
...
sfetch_response_t :: struct {
	...
	path:        cstring,         // the original filesystem path of the request
	user_data:   rawptr,          // pointer to read/write user-data area
...
sfetch_request_t :: struct {
	...
	callback:   proc "c" (^sfetch_response_t), // response callback function pointer (required)
...
foreign lib {
	...
	sfetch_handle_valid :: proc(h: sfetch_handle_t) -> bool ---
