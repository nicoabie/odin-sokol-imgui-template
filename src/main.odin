	...
	if state.failed {
		...
		for node_index in 0..<state.scene.num_nodes {
			...
			for i in 0..<mesh.num_primitives {
				...
				sg.apply_uniforms(sgltf.UB_light_params, sg.Range{&state.point_light, size_of(state.point_light)})
				...
				if mat.is_metallic {
					...
					sg.apply_uniforms(sgltf.UB_metallic_params, sg.Range{&mat.metallic.fs_params, size_of(mat.metallic.fs_params)})
				}
